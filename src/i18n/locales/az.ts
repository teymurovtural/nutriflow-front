const az = {
  nav: {
    howItWorks: "Necə işləyir",
    pricing: "Qiymətlər",
    about: "Haqqımızda",
    faq: "FAQ",
    login: "Daxil ol",
    getStarted: "Başla",
    language: "Dil",
  },
  hero: {
    badge: "Fərdiləşdirilmiş Qidalanma + Katering Çatdırılması",
    titleStart: "Fərdiləşdirilmiş ",
    titleHighlight: "Qidalanma",
    titleEnd: ", Sadələşdirildi",
    subtitle:
      "NutriFlow qidalanma planınızı real məlumatlara əsaslanaraq qurur, trendlərə deyil.",
    cta: "Səyahətinizi başladın",
    viewPricing: "Qiymətlərə baxın",
  },
  whatIsNutriflow: {
    title: "NutriFlow nədir?",
    description:
      "NutriFlow peşəkar qidalanma planlamasını rahat yemək çatdırılması ilə birləşdirir. Sertifikatlı diyetisyenlər tərəfindən hazırlanan fərdi aylıq menyuları birbaşa evinizdə alın.",
    cards: {
      healthFirst: {
        title: "Sağlamlıq Önündə",
        description:
          "Hər menyu sizin unikal sağlamlıq profilinizə, qidalanma məhdudiyyətlərinizə və qida məqsədlərinizə uyğun hazırlanır.",
      },
      expertDietitians: {
        title: "Mütəxəssis Diyetisyenlər",
        description:
          "Sertifikatlı qidalanma mütəxəssisləri dəqiq makro və kalori izləməsi ilə aylıq yemək planınızı hazırlayır.",
      },
      dailyDelivery: {
        title: "Gündəlik Çatdırılma",
        description:
          "Hər gün hazır və çatdırılan təzə yeməklər. Çatdırılma statusunu real vaxt rejimində izləyin.",
      },
    },
  },
  features: {
    personalizedPlans: {
      title: "Fərdiləşdirilmiş Yemək Planları",
      description:
        "Hər yemək xüsusi olaraq sizin üçün hazırlanır. Sertifikatlı diyetisyenlərimiz sağlamlıq məlumatlarınızı, qidalanma məhdudiyyətlərinizi və məqsədlərinizi təhlil edərək mükəmməl fərdi menyu yaradır.",
      bullets: [
        "Sağlamlıq məqsədlərinizə uyğunlaşdırılıb",
        "Qidalanma məhdudiyyətlərini nəzərə alır",
        "Dəqiq makro və kalori izləməsi",
        "Aylıq menyunun yenilənməsi",
      ],
      imgAlt: "Qida planlaması",
    },
    freshIngredients: {
      title: "Təzə, Keyfiyyətli İnqrediyentlər",
      description:
        "Yeməkləriniz üçün yalnız ən keyfiyyətli, ən təzə inqrediyentlər istifadə edirik. Hər yemək peşəkar aşpazlar tərəfindən gündəlik hazırlanır.",
      bullets: [
        "Yerli mənşəli məhsullar",
        "Premium keyfiyyətli zülallar",
        "Süni qoruyucusuz",
        "Gündəlik aşpaz hazırlaması",
      ],
      imgAlt: "Təzə sağlam salat",
    },
    dailyDelivery: {
      title: "Rahat Gündəlik Çatdırılma",
      description:
        "Ərzaq mağazası və yemək hazırlama məşəqqətini unut. Yeməkləriniz hər gün qapınıza təzə çatdırılır. İdarəetmə panelinizdən real vaxtda izləyin.",
      bullets: [
        "Gündəlik qapıya çatdırılma",
        "Real vaxt izləmə",
        "Çevik çatdırılma pəncərələri",
        "Ekoloji qablaşdırma",
      ],
      imgAlt: "Sağlam qida çatdırılması",
    },
  },
  howItWorks: {
    title: "Necə işləyir",
    subtitle:
      "NutriFlow qidalanma planınızı real məlumatlara əsaslanaraq qurur, trendlərə deyil.",
    tagline: "Sağlam qidalanma strukturlu hiss etdirilməlidir, stresli yox.",
    steps: [
      {
        title: "Özünüz haqqında danışın",
        description:
          "Məqsədlərinizi, qidalanma üstünlüklərini, allergiyaları, fəaliyyət səviyyənizi və həyat tərzinizi paylaşın.",
      },
      {
        title: "Ağıllı plan yaradılması",
        description:
          "Sistemimiz kalori ehtiyaclarınıza, makrolarınıza və sağlamlıq hədəflərinizə uyğun balanslaşdırılmış yemək planı yaradır.",
      },
      {
        title: "Təzə yemək çatdırılması",
        description:
          "Yeməklər cədvəlinizə uyğun hazırlanır və çatdırılır, qızdırmağa hazır.",
      },
      {
        title: "İzlə və optimallaşdır",
        description:
          "Tətbiqdə irəliləyişi izləyin. İstənilən vaxt məqsədləri dəyişdirin. Planınız sizinlə birlikdə inkişaf edir.",
      },
    ],
  },
  pricingPreview: {
    title: "Sadə, şəffaf qiymətlər",
    subtitle: "Bir plan — hər şey daxildir",
    planName: "Premium Plan",
    planDescription:
      "Ciddi nəticələr üçün tam fərdiləşdirilmiş qidalanma təcrübəsi.",
    price: "1500",
    currency: "AZN/ay",
    features: [
      "Məqsəd və sağlamlıq məlumatlarınıza uyğun fərdi yemək planı",
      "Gündəlik təzə yemək çatdırılması",
      "Makro və kalori dəqiqliyi",
      "Planın davamlı tənzimlənməsi",
      "Tətbiqdaxili irəliləyiş izləməsi",
      "Prioritet müştəri dəstəyi",
    ],
    note: "Bu adi abunəlik deyil. Arxasında əməliyyat mükəmməlliyi olan strukturlaşdırılmış, məlumat əsaslı qidalanmadır.",
    noHiddenFees:
      "Gizli ödənişlər yoxdur. Uzunmüddətli bağlılıq yoxdur. Yalnız performansa və ölçülə bilən nəticələrə fokus.",
    cta: "Başla",
  },
  whoItsFor: {
    title: "Kimin üçündür",
    cards: [
      {
        title: "Məşğul Peşəkarlar",
        description:
          "Yemək planlaması və bişirməyə vaxt sərf etməyin. Ən vacib işlərə fokuslanın.",
      },
      {
        title: "Sağlamlığa Diqqət Edənlər",
        description:
          "Mütəxəssislər tərəfindən hazırlanmış yeməklərlə fitness və sağlamlıq məqsədlərinizə çatın.",
      },
      {
        title: "Xüsusi Qidalanma Ehtiyacları",
        description:
          "Allergiyalar, məhdudiyyətlər və tibbi qidalanma tələblərinizi nəzərə alırıq.",
      },
    ],
  },
  aboutUs: {
    title: "Elm üzərində qurulmuşdur. Texnologiya ilə gücləndirilib.",
    description:
      "NutriFlow sağlam qidalanmadan təxmini aradan qaldırmaq üçün yaradılmışdır. Qidalanma ekspertizasını, ağıllı alqoritmləri və əməliyyat dəqiqliyini birləşdiririk.",
    missionLabel: "Missiyamız:",
    mission:
      "Fərdiləşdirilmiş qidalanmanı əlçatan, miqyaslana bilən və davamlı etmək.",
    belief:
      "İnanırıq ki, qidalanma məqsədlərinizi dəstəkləməlidir, həyatınızı çətinləşdirməməlidir.",
  },
  faq: {
    title: "Tez-tez verilən suallar",
    items: [
      {
        q: "Planı necə seçim?",
        a: "Məqsədinizlə başlayın — çəki itirilməsi, əzələ artımı və ya balanslaşdırılmış qidalanma. İstənilən vaxt dəyişdirə bilərsiniz.",
      },
      {
        q: "Yeməkləri dəyişdirə bilərəmmi?",
        a: "Bəli. Tətbiqdə həftəlik seçimləri fərdiləşdirin.",
      },
      {
        q: "Xüsusi pəhrizlərə dəstək varmı?",
        a: "Vegetarian, yüksək zülallı və allergen həssaslığına uyğun variantlar təklif edirik.",
      },
      {
        q: "Çatdırılma necə işləyir?",
        a: "Yeməklər seçilmiş cədvələ uyğun olaraq təzə çatdırılır.",
      },
      {
        q: "Abunəliyi ləğv etmək istəsəm?",
        a: "Profil parametrlərindən istənilən vaxt abunəliyi ləğv edə bilərsiniz. Abunəlik ödəniş dövrünün sonuna qədər aktiv qalır.",
      },
    ],
    contact: "Hələ suallarınız var?",
  },
  cta: {
    title: "Qidalanmanızı dəyişdirməyə hazırsınız?",
    subtitle: "Sağlamlıq yolunda yüzlərlə məmnun müştəriyə qoşulun",
    button: "Bu gün başlayın",
  },
  footer: {
    tagline: "Fərdiləşdirilmiş qidalanma, hər gün çatdırılır.",
    company: "Şirkət",
    support: "Dəstək",
    contactTitle: "Əlaqə",
    links: {
      about: "Haqqımızda",
      careers: "Karyera",
      blog: "Bloq",
      press: "Mətbuat",
      faq: "FAQ",
      contactUs: "Bizimlə əlaqə",
      privacy: "Məxfilik siyasəti",
      terms: "İstifadə şərtləri",
    },
    copyright: "© 2026 NutriFlow. Bütün hüquqlar qorunur.",
  },
} as const;

export default az;
